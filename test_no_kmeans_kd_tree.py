...
def reverse_binary_matrix(list_of_iteration, dataframe_to_match,column):
    list_binary = []
    for value in list_of_iteration:
...
for configuration in tqdm(np.arange(0,0.85,0.05)):
    ...
    for patient in tqdm(range(number_of_patients)):
        ...
        if patient < number_of_patients/patient_phenotypes:
            ...
            data_generated = generate_data(size_list,proba_tupple_2)
            equivalent = np.array(list(itertools.product([0, 1], repeat=2)))
            conversion = compute_binary_matrix(equivalent)
            binary_result = pd.DataFrame(equivalent)
            binary_result['Match_number'] = pd.Series(conversion)
        ...
        frame_binary.to_csv(path_to_store_patient_frame + f'/Generated_binary_configuration_{configuration}_file_for_patient_N°{str(patient).zfill(3)}.csv')


    
    dirs = [path_to_store_patient_frame]
    for dir in dirs:
        files = glob(f'{dir}/Generated_binary_configuration_{configuration}_*.csv')
    ...
    reference_dataframe.to_csv(path_to_store_frame + f'/Data_for_LDA_from_generate_data_with_n_{number_of_cluster}_configuration_{configuration}.csv')
    ...
    #for runrun in range(17, 18):
        ...
        raw_data_T4 = pd.read_csv(path_to_store_frame + f'/Data_for_LDA_from_generate_data_with_n_{number_of_cluster}_configuration_{configuration}.csv' ,index_col= 'Cluster')
        ...
        plt.title(f'Correlation matrix for topic N°%s, and N {number_of_cluster} for clustering configuration {configuration}' %runrun)
        ...
        plt.savefig(path_to_store_figures + f'/corr_matrix_on_binary_data_generated_topic_%s_N_{number_of_cluster}_for_kmeans_configuration_{configuration}.png'%runrun)
        (pd.DataFrame(norm_theta)).to_csv(path_to_store_frame + f'/LDA_DATAFRAME_Topic_{runrun}_configuration_{configuration}.csv')
        plt.close()
        ...
        frame_binary_with_threshold.groupby(['Phenotype','Hierarchical_clustering']).size().unstack(fill_value=0).to_csv(path_to_store_frame + f'/Confusion_matrix_Topic_{runrun}_with_n_{number_of_cluster}_configuration_{configuration}.csv')
        ...
        plt.savefig(path_to_store_figures + f'/Euclidian_distance_matrix_sorted_by_Hierarchical_Clustering_for_topic_%s_and_N_{number_of_cluster}_configuration_{configuration}.png'%runrun)
        plt.close()